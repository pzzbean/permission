...
<script type="text/javascript">
   $(function () {
       ...
       var aclModuleMap = {}; //存储Map格式来缓存所有权限模块的信息
       var aclMap = {}; //存储Map格式来缓存所有权限的信息
       var optionStr = "";
       ...
       function bindAclModuleClick(){
            ...
            $(".aclModule-name").click(function(e){
                e.preventDefault();
                e.stopPropagation();
                var aclModuldId = $(this).attr("data-id")
                handleAclModuleSelected(aclModuldId);
           ...
           $(".aclModule-edit").click(function (e) {
               ...
               $("#dialog-aclModule-form").dialog({
                   ...
                   open: function(event,ui){
                       ...
                       $("#aclModuleForm")[0].reset();
                       $("#parentId").html(optionStr);
                       $("#aclModuleId").val(aclModuldId);
                       var targetAclModule = aclModuleMap[aclModuldId];
                       if(targetAclModule){
            ...
            if (result.ret){
                if(result.data.total > 0){
                    var rendered = Mustache.render(aclListTemplate, {
                        ...
                        "showType": function(){
                            return this.type == 1 ? "菜单" :(this.status == 2 ? "按钮" : "其他");
                        },
                        "bold": function(){
                            return function (text, render) {
