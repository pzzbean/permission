...
<script type="text/javascript">
 $(function(){
     ...
     var selectedUserTemplate = $("#selectedUsersTemplate").html();
     Mustache.parse(selectedUserTemplate);

     var unselectedUserTemplate = $("#unSelectedUsersTemplate").html();
     Mustache.parse(unselectedUserTemplate);
     ...
     function loadRoleList(){
        $.ajax({
            ...
            success: function(resulet){
                if(resulet.ret){
                    var rendered = Mustache.render(roleListTemplate,{roleList:resulet.data});
                    $("#roleList").html(rendered);
                    bindRoleClick();
                    $.each(resulet.data, function (i, role) {
                        roleMap[role.id] = role;
                    })
                }else{
                    showMessage("加载角色列表", resulet.msg, false);
                }
            }
        })
     }
                 ...
                 buttons: {
                     "更新":function (e) {
                         ...
                         updateRole(false,function(data){
                             ...
                             showMessage("编辑角色", data.msg, false);
                         })
                     },
                     "取消":function(){
                         $("#dialog-role-form").dialog("close");
     ...
     $(".saveRoleAcl").click(function(e){
         ...
         $.ajax({
             ...
             data: {roleId: lastRoleId, aclIds: getTreeSelectedId()},
             type: "post",
             success: function (result) {
                if (result.ret){
     ...
     function loadRoleUser(selectedRoleId) {
         $.ajax({
             ...
             success: function(result){
                if(result.ret){
                    var renderedSelect = Mustache.render(selectedUserTemplate, {userList: result.data.selected});
                    var renderedUnSelect = Mustache.render(unselectedUserTemplate, {userList: result.data.unselected});
     ...
     $(".saveRoleUser").click(function(e){
         ...
         $.ajax({
             ...
             data: {roleId: lastRoleId, userIds: $("#roleUserList").val().join(",")},
             type: "post",
             success: function (result) {
                 if (result.ret){
                     showMessage("保存角色与角色的关系", "操作成功", false)
                 } else{
                     showMessage("保存角色与角色的关系", result.msg, false)
                 }
             }
         });
     })
